...
class DatabaseHelper
{
    ...
    public static function _init_mysql($config = [])
    {
        if (empty($config)) {
        ...
        if ( !self::$conn ) {
            ...
            if(mysqli_connect_errno()) {
                ...
                echo Utils::colorize(date("H:i:s") . " {$errmsg}\n\n", 'fail');
     ...
     */
    public static function reset_connect($config = [])
    {
    ...
    public static function update($table = '', $data = [], $where = null, $return_sql = false)
    {
        $sql = "UPDATE `{$table}` SET ";
        foreach ($data as $k => $v) {
            $v = stripslashes($v);
